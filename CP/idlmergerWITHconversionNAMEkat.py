...
def create_cellfile(locpath, spotoutfile, celloutfile):
    ...
    # write data for cells with spots
    with open(join(locpath, spotoutfile), 'r') as f:
        for i, line in enumerate(f):
            if i:
                linedata = line.split()
                data.append(linedata)
                cell_ID = str(linedata[6])+"_"+str(linedata[4])
                celldict[cell_ID] = [0.0, 0, 0] # intensity, spots, RNAs
                print "cell_ID =", cell_ID
                cell_IDs.add(cell_ID)

    for ID in cell_IDs:
        celldict[ID][0] = str(sum(float(linedata[2]) for linedata in data if str(linedata[6])+"_"+str(linedata[4])==ID)) # intensities
        celldict[ID][1] = str(sum(int(linedata[5]) for linedata in data if str(linedata[6])+"_"+str(linedata[4])==ID)) # spots
        celldict[ID][2] = str(sum(int(linedata[7]) for linedata in data if str(linedata[6])+"_"+str(linedata[4])==ID)) # RNAs
